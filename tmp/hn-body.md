...
> **Disclosure:** I work on RocketRide, an open source AI pipeline runtime, which is one of the tools this post talks about. So I have a stake in one of the five layers below, and this is not an independent assessment of the acquisitions described. Every external claim is linked to its source, and the post says which claims rest on reporting that no company has confirmed.
...
RocketRide is the orchestration layer, and that last question is the one it's built to answer with yes. The runtime is MIT licensed, it runs on your own infrastructure, and a pipeline is portable JSON rather than a hosted object.
...
Then run the same audit on RocketRide. The `base_url` field, the governance document and the node definitions are all in [rocketride-server](https://github.com/rocketride-org/rocketride-server) under MIT, so none of the claims above need taking on trust. Self-host when owning the infrastructure is part of the job, and use [Cloud](https://cloud.rocketride.ai/) when the pipeline is the work and operating the platform is not; either way the artifact is yours. If you run the audit and find a layer I missed, I'm in the [Discord](https://discord.gg/PMXrtenMsY).
